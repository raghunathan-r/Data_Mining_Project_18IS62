...
print("\nBinarized countries [France - 0]: \n", label_bin.fit_transform(data_set['Country']))

# trying to put all the data into a output file
output_dataset = pd.read_csv("./Sample_Salaries_Data_Set_After_Binarisation.csv")

# (data_set['age'].value).reshape(-1, 1)

# output_dataset['Bin_age'] = binarizer_1.fit_transform((data_set['age'].value).reshape(-1, 1))
# output_dataset['Bin_salary'] = binarizer_2.fit_transform(y)

output_dataset['Bin_age'] = label_bin.fit_transform(data_set['Country'])
print("\n\nOutput DataSet :\n", output_dataset)